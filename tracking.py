...
parser.add_argument("--object_detector_model_dir", default="./faster_rcnn_resnet101_kitti_2018_01_28", type=str)
parser.add_argument("--path_to_reid_model", default="./mars-small128.pb", type=str)
...
parser.add_argument("--reid_sim_threshold", default=0.3, type=float)
...
while True:
    ...
    for result in results:
        bbox = result[0]
        id = result[1]
        cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[1], bbox[2]), (255, 255, 255), 1)
        cv2.putText(img, "Id:" + str(id), (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color=(255, 255, 255), thickness=1)

    for detection in detections:
        bbox = detection.bbox
        cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[1], bbox[2]), (0, 255, 0), 1)
